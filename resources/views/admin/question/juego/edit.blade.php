...
<div class="bg-dark text-white font-bold rounded">
    <a href="{{route('admin.question.juego.index')}}"><button class="btn btn-info m-3">Volver al menú principal</button></a>
</div>
<div class="card">
    ...
    @endif
    <div class="card-body">
        <form action="{{route('admin.question.juego.update', $juego)}}" method="POST" enctype="multipart/form-data">
            @csrf 
            {{method_field('PUT')}}

            <div class="form-group">
                <label>Enunciado de pregunta:</label>
                <input type="text" name="title" id="title" placeholder="Ingrese el enunciado de pregunta" value="{{$juego->title}}" class="form-control" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="slug" id="slug" placeholder="Slug de la pregunta" value="{{$juego->slug}}" class="form-control" required readonly>
            ...
            <div class="form-group">
                ...
                <select name="evaluation_id" id="evaluation_id" class="form-control" required>
                    @foreach ($evaluations as $evaluation)
                        <option value="{{$evaluation->id}}" @if($evaluation->id==$juego->evaluation_id) selected @endif>{{$evaluation->name}}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <br>
                <textarea name="rule" id="rule" class="form-control" value="{{$juego->rule}}">{{$juego->rule}}</textarea>
            ...
            <div class="form-group">
                <label>Ingrese la oración visible al usuario:</label>
                <br>
                @foreach ($juego->answers as $answer)
                    <br>
                    <input type="text" name="visible_answer" placeholder="Ingrese la oración visible al usuario" value="{{$answer->visible_answer}}" class="form-control" required>
                    <br>
                @endforeach
                

                <!-- MENSAJE DE ERROR DE VALIDACION -->
                @error('visible_answer')
                    <small class="text-danger">{{$message}}</small>
                @enderror
            </div>

            <div class="form-group">
                <label>Ingrese la palabra por adivinar:</label>
                <br>
                @foreach ($juego->answers as $answer)
                    <br>
                    <input type="text" name="answer" placeholder="Ingrese la palabra a adivinar" value="{{$answer->answer}}" class="form-control" required>
                    <br>
                @endforeach
            ...
            <div class="form-group">
                ...
                <br>
                @foreach ($juego->indications as $indication)
                    <br>
            ...
            <div class="form-group">
                ...
                <br>
                @foreach ($juego->justifications as $justification)
                    <br>
