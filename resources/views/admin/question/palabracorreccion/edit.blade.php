...
<div class="bg-dark text-white font-bold rounded">
    <a href="{{route('admin.question.palabracorreccion.index')}}"><button class="btn btn-info m-3">Volver al menú principal</button></a>
</div>
<div class="card">
    ...
    @endif
    <div class="card-body">
        <form action="{{route('admin.question.palabracorreccion.update', $palabracorreccion)}}" method="POST">
            @csrf 
            {{method_field('PUT')}}

            <div class="form-group">
                <label>Enunciado de pregunta:</label>
                <input type="text" name="title" id="title" placeholder="Ingrese el enunciado de pregunta" value="{{$palabracorreccion->title}}" class="form-control" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="slug" id="slug" placeholder="Slug de la pregunta" value="{{$palabracorreccion->slug}}" class="form-control" required readonly>
            ...
            <div class="form-group">
                ...
                <select name="evaluation_id" id="evaluation_id" class="form-control" required>
                    @foreach ($evaluations as $evaluation)
                        <option value="{{$evaluation->id}}" @if($evaluation->id==$palabracorreccion->evaluation_id) selected @endif>{{$evaluation->name}}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <br>
                <textarea name="rule" id="rule" class="form-control" value="{{$palabracorreccion->rule}}">{{$palabracorreccion->rule}}</textarea>
            ...
            <div class="form-group">
                <label>Ingrese las palabras correctas y las palabras visibles:</label>
                <br>
                <div class="row">
                    <div class="col">
                ...
                @foreach ($palabracorreccion->answers as $key=>$answer)
                    <br>
                    <div class="row">
                        <div class="col">
